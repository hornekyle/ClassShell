...
add_custom_target("Definitions" ALL)

set(SCRIPT_FILE "${CMAKE_SOURCE_DIR}/Tooling/Generators/Definitions.py")
set(DEFINITIONS_SHEET "Tooling/Sheets/Definitions.ods")
set(DEFINITIONS_SHEET_FULL "${CMAKE_SOURCE_DIR}/${DEFINITIONS_SHEET}")

execute_process(
	COMMAND
		python "-B" "${SCRIPT_FILE}"
		"${DEFINITIONS_SHEET_FULL}" "--modules"
	OUTPUT_VARIABLE
		MODULES_LIST
	WORKING_DIRECTORY
		"${CMAKE_BINARY_DIR}"
...
foreach(MODULE ${MODULES_LIST})
	...
	set(TARGET_NAME "Include-+-${MODULE}-+-Definitions")
	
	set(TEX_FILE "Includes/${MODULE}/Definitions.tex")
	...
	set(TARGET_ALERT " :  <source>:${DEFINITIONS_SHEET}∈${MODULE} →  <build>:${TEX_FILE}")
	...
	add_custom_command(
		...
		COMMAND
			python
			>> "${LOG_FILE_FULL}"
			"-B" "${SCRIPT_FILE}" "${DEFINITIONS_SHEET_FULL}"
			"--build" "${MODULE}" "${TEX_FILE_FULL}"
		MAIN_DEPENDENCY
			"${DEFINITIONS_SHEET_FULL}"
		DEPENDS
			"${SCRIPT_FILE}"
		COMMENT
			"${TARGET_ALERT}"
	...
	add_dependencies("Definitions" "${TARGET_NAME}")
